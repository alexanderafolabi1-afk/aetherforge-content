...
- `content-queue.json` (not committed yet) — a JSON array of `ScheduledPost` objects. Written directly by the Command Deck's **GitHub Sync** (Connectors → GitHub Sync) via the GitHub Contents API, and read/updated by the n8n workflow. Nobody edits this by hand anymore — see [`AUTOMATION_GUIDE.md`](../AUTOMATION_GUIDE.md).
